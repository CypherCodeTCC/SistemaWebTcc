...
import { BookInfo, Button, Checkout, Container, ContainerAddress, ContainerButton, ContainerPurchase, Input, Items, SubTitleItems, Title } from "./finalizePurchaseStyle";
...
export default function FinalizePurchase() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const res = await axios.get(`https://node-routes-mysql.vercel.app/client/${localStorage.getItem(
                    "userId"
                )}`);
                const userEmail = res.data.Email ? res.data.Email : JSON.parse(userData).email;
                setUser({
                    ...res.data,
                    Email: userEmail,
                });
    ...
    useEffect(() => {
        const fetchAddress = async () => {
            try {
                const res = await axios.get(`https://node-routes-mysql.vercel.app/client/address/${localStorage.getItem(
                    "userId"
                )}`);
                setAddress(res.data);
            }
            catch (err) {
                toast.error("Erro ao encontrar endereço do usuário. Tente mais tarde.", {
                    closeOnClick: true,
                });
                ...
                <ContainerPurchase>
                    {itemsOnCart.map((itemOnCart) => (
                        <Items key={itemOnCart.id}>
                            ...
                            <SubTitleItems>R${(itemOnCart.price * cartItems[itemOnCart.id]).toFixed(2)}</SubTitleItems>
                        </Items>
                    ))}
                    <Checkout>
                        <Infos>
