...
export default function SignUp() {
    ...
    const gerarCPF = () => {
        ...
        let cpf = Array.from({ length: 11 }, () => Math.floor(Math.random() * multiplicador)).join('');
        return cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');
      };

    const gerarCEP = () => {
        const multiplicador = 9;
        return `${Math.floor(Math.random() * multiplicador)}${Math.floor(Math.random() * multiplicador)}${Math.floor(Math.random() * multiplicador)}${Math.floor(Math.random() * multiplicador)}${Math.floor(Math.random() * multiplicador)}-${Math.floor(Math.random() * multiplicador)}${Math.floor(Math.random() * multiplicador)}${Math.floor(Math.random() * multiplicador)}`;
    };
    ...
    const preencherComDadosAleatorios = () => {
        if (dadosJson.length > 0) {
            ...
            nomeRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Nome.split(' ')[0];
            sobrenomeRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Nome.split(' ')[1];
            cpfRef.current.value = gerarCPF();
            telefoneRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Telefone;
            emailRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Email;
            cepRef.current.value = gerarCEP();
            logradouroRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Logradouro;
            numeroEndRef.current.value = gerarNumero();
            complementoRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Complemento;
            nomeCidRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].NomeCid;
            ufRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Uf;
            senhaRef.current.value = dadosJson[Math.floor(Math.random() * dadosJson.length)].Senha;
            confirmaSenhaRef.current.value = senhaRef.current.value;

            console.log(`Email: ${emailRef.current.value}, Senha: ${senhaRef.current.value}`);
        }
    };
            ...
            <Container>
                ...
                <div className='container-form'>
                    ...
                    <div className='input-text'>
                        <h4>Primeiro Nome</h4>
                        <input type="text" placeholder='Ex: Juan' name="Nome" ref={nomeRef} onChange={(e) => {
                    ...
                    <div className='input-text'>
                        <h4>CPF</h4>
                        <input type="text" placeholder='Ex: 123.456.789-11' name="Cpf" ref={cpfRef} />
                    </div>
                    <div className='input-text'>
                        <h4>Telefone</h4>
